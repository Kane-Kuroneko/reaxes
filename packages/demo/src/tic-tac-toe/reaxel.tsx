...
export const reaxel_tic_tac_toe = reaxel(() => {
	...
	Reaxes.obsReaction( () => {
		...
		while( x<2 || y<2 ){
			...
			if(checkerboard[y][x] === player){
				let _x1 = x,_y1 = y,_x2 = x,_y2 = y;
				/*向右查找*/
				const row = (player === checkerboard[y]?.[x+1]) && (player === checkerboard[y]?.[x+2]);
				/*右下查找*/
				const slash = (player === checkerboard[y+1]?.[x+1]) && (player === checkerboard[y+2]?.[x+2]);
				/*左下查找*/
				const bslash = (player === checkerboard[y+1]?.[x-1]) && (player === checkerboard[y+2]?.[x-2]);
				/*向下查找*/
				const down = (player === checkerboard[y+1]?.[x]) && (player === checkerboard[y+2]?.[x]);
				...
				if(row || slash || bslash || down){
					setState({
						winner : player,
						winLine : [{x,y},{x:_x1,y:_y1},{x:_x2,y:_y2}],
					});
					return;
				}else {
					x < 2 ? x++ : ( y++ , x = 0 );
	...
	const returning = {
		store,
		get checkerboard(){ return store.checkerboard; },
		/*放置棋子*/
		place,
		/*重新开始一局*/
		play(){
		...
		get winner(){ return store.winner},
