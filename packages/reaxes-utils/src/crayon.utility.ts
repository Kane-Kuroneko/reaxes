...
/*把驼峰css key 改为短横连*/
const transformCSSHumpToLowercase = ( key: string ) => {
	return key.replace( /([A-Z])/g , '-$1' ).toLowerCase();
};
/*将驼峰css key对象改为短横连*/
const batchTransformCSSHumpToLowercase = ( object: { [key in keyof CSSStyleDeclaration]: any } ) => {
	const obj: { [key in keyof Partial<CSSStyleDeclaration>]: any } = {};
...
const argumentsAgent = ( secondary , ...args ) => args.reduce( ( accu , arg , index ) => (
	accu[0] += typeof arg === 'string' ? arg : (
		accu.push( arg ), '%o'
	), accu
) , [ '%c' , secondary ] );
 ...
 */
export const crayon = new Proxy( ( cssProperties: Partial<CSSStyleDeclaration> = {} ) => {
	...
	get : ( target , propKey: any , receiver ) => {
		...
		const validKeys = [
			'trace' , 'warn' , 'log' , 'info' , 'error' , 'debug' ,
			'assert' , 'clear' , 'count' , 'countReset' , 'dir' ,
			'dirxml' , 'table' ,
		] as const;
		
		if( validKeys.includes( propKey ) ) {
...
if( 0 ) {
	...
	crayon.warn['#eeeeee']( 'dsdsdsdsds' , { b : 222 } );
